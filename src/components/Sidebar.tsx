      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="p-4">
          <div className="text-center mb-4">
            <div className="text-3xl font-bold text-pink-800">{language === 'ca' ? 'Octubre' : 'Octubre'}</div>
            <div className="text-5xl font-bold text-amber-600">2025</div>
          </div>
          <div className="grid grid-cols-7 gap-1 text-center text-sm">
            {['L', 'M', 'X', 'J', 'V', 'S', 'D'].map((day, i) => (
              <div key={i} className="font-semibold text-pink-600">{day}</div>
            ))}
            {[...Array(31)].map((_, i) => {
              const day = i + 1;
              ...
              return (
                <div
                  ...
                  className={`py-1 rounded ${
                    isActivity ? 'bg-amber-500 text-white font-bold' : 'text-pink-700'
                  }`}
                >
                  {day}
                </div>
        ...
        <div className="p-4 space-y-4 max-h-80 overflow-y-auto">
          {activities.map((activity, index) => (
            <div key={index} className="border-l-4 border-amber-500 pl-3 py-2 hover:bg-pink-50 transition-colors">
              ...
              <div className="font-bold text-pink-800">{activity.title[language]}</div>
              <div className="text-sm text-pink-600">{activity.description[language]}</div>
            </div>
          ))}
        </div>
      </div>
